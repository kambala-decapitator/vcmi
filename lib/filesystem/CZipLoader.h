...
public:
	 ...
	 */
	CZipStream(std::shared_ptr<CIOApi> api, const bfs::path & archive, unz64_file_pos filepos);
	~CZipStream();
...
class DLL_LINKAGE CZipLoader : public ISimpleResourceLoader
{
	...
	bfs::path archiveName;
	std::string mountPoint;

	std::unordered_map<ResourceID, unz64_file_pos> files;

	std::unordered_map<ResourceID, unz64_file_pos> listFiles(const std::string & mountPoint, const bfs::path &archive);
public:
	CZipLoader(const std::string & mountPoint, const bfs::path & archive, std::shared_ptr<CIOApi> api = std::shared_ptr<CIOApi>(new CDefaultIOApi()));
...
namespace ZipArchive
{
	...
	std::vector<std::string> DLL_LINKAGE listFiles(bfs::path filename);

	/// extracts all files from archive "from" into destination directory "where". Directory must exist
	bool DLL_LINKAGE extract(bfs::path from, bfs::path where);

	///same as above, but extracts only files mentioned in "what" list
	bool DLL_LINKAGE extract(bfs::path from, bfs::path where, std::vector<std::string> what);
}
